...
export interface IAPI {
	...
	updateField: <EntityType = BaseEntity>(id: string, locale: string, field: string, value: string) => Promise<EntityType>
...
export class API implements IAPI {
	...
	async savedSearchesIncludeEntity(searchIds: string[], entityId: string) {
		...
		const req = new Request(this.constructRequestUrl('entities', params), {headers});
		const resp = await wrappedFetch<ListEntitiesResponse<BaseEntity>>(req);
		return resp.count === 1;
	}

	async updateField<EntityType = BaseEntity>(id: string, locale: string, field: string, value: string) {
