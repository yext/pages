...
describe("loadTemplateModules", () => {
  ...
  it("loads and transpiles raw templates", async () => {
    ...
    const functionModules = await loadFunctionModules(functionFile, true, projectStructure);
  ...
  it("loads transpiled templates", async () => {
    ...
    const functionModules = await loadFunctionModules(functionFile, false, projectStructure);
    commonTests(functionModules, "param-47853");
  });

  const commonTests = (functionModules: FunctionModuleCollection, functionName: string) => {
    expect(functionModules.get(functionName)).toBeTruthy();
    expect(functionModules.get(functionName)?.config.name).toEqual(functionName);
    expect(functionModules.get(functionName)?.config.event).toEqual("API");
    expect(functionModules.get(functionName)?.config.functionName).toEqual("default");
